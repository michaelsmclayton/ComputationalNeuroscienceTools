...
def get_total_number_of_synapses(net_dict):
    ...
    N_full = net_dict['N_full'] # Get neuron numbers in each layer
    number_N = len(N_full) # Get the number of populations
    conn_probs = net_dict['conn_probs'] # Get connection probabilities
    scaling = net_dict['N_scaling'] # Get number scaling value

    # Get outer product of neuron numbers (giving all the possible synapses between two given populations)
    ...
    ''' I don't understand this bit of code. It seems to be a very complex way of simply
    multiplying the connection probability between two given popopulation, with the total number
    of possible connections betweeen those populations (i.e. N_pop1 * N_pop2)? I have therefore
    simplified the code (although this may be unwise)
    '''
    # K = (prod * conn_probs * scaling).astype(int) # Simplification of above?
